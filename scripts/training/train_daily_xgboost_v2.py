"""
Train rectified daily macro XGBoost ranking models using Walk-Forward Validation.
- Loads data/ranking_data_upstox_daily_5y.csv (regenerated with daily features)
- Automatically detects GPU (CUDA) availability and falls back to CPU if needed
- Performs 4-fold Walk-Forward Validation to ensure honest, regime-diverse performance estimates with NO test set leakage
- Trains final production models on maximum historical context, early stopped on the latest months
- Saves final models, walk-forward metadata, and scaler to models/daily_xgb_v2/
"""
...
print("=" * 60)
print("RECTIFIED DAILY MACRO XGBOOST RANKER TRAINING PIPELINE (V2)")
print("Walk-Forward Validation with 3-Way Splits & Extended Early Stopping")
print("=" * 60)
...
y_returns = df['Next_Day_Return'].values
...
# ========================================
# TRAINING HYPERPARAMETERS (RECTIFIED V2)
# ========================================
params = {
    'objective': 'rank:pairwise',
    'eta': 0.01,              # Rectified: Slower learning rate for stable training
    'max_depth': 5,           # Shallow trees for better generalization
    'subsample': 0.8,
    'colsample_bytree': 0.8,
    'alpha': 2.0,             # Rectified: Higher L1 regularization to prevent noise fitting
    'lambda': 4.0,            # Rectified: Higher L2 regularization
    'min_child_weight': 40,   # Rectified: Restrict splits on small noisy nodes
    'random_state': 42,
    'verbosity': 0,
    'eval_metric': 'ndcg@5',  # Daily macro target: top 5 selections
    'ndcg_exp_gain': False,
    'tree_method': 'hist',
    'device': device
}
...
# ========================================
# Spanning 5 years from 2021-05 to 2026-05
folds_config = [
    {
        'fold': 1,
        'train_months': unique_months[:30],  # First 30 months (~2.5 years)
        'val_months': unique_months[30:36],  # Next 6 months
        'test_months': unique_months[36:42]  # Next 6 months
    },
    {
        'fold': 2,
    ...
    },
    {
        ...
        'test_months': unique_months[54:]  # Up to 2026-05
    }
]
...
# ========================================
print("\n" + "=" * 60)
print("RUNNING WALK-FORWARD VALIDATION ON RECTIFIED DAILY MACRO TRENDS")
print("=" * 60)
...
for cfg in folds_config:
    ...
    # Train Long model with increased patient iterations (early stopped)
    ...
    # Train Short model with increased patient iterations (early stopped)
...
avg_long_best_iter = np.mean([r['long_best_iter'] for r in walk_forward_results])
avg_short_best_iter = np.mean([r['short_best_iter'] for r in walk_forward_results])

print("\n" + "=" * 60)
print("AGGREGATE WALK-FORWARD VALIDATION RESULTS (RECTIFIED V2)")
print("=" * 60)
print(f"Averaged over {len(folds_config)} temporal test folds:")
print(f"  Average Best Iterations:")
print(f"    Long Model  : {avg_long_best_iter:.1f}")
print(f"    Short Model : {avg_short_best_iter:.1f}")
...
# Train on all months up to 2026-03
# Validate on 2026-04 and 2026-05
...
print("\n" + "=" * 60)
print("RECTIFIED DAILY MACRO XGBOOST TRAINING COMPLETE")
