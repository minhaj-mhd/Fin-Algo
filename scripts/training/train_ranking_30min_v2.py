"""
Train standalone 30-minute XGBoost ranking models on 3 years of Upstox V3 data.
Uses Walk-Forward Validation with strict time-series splits.
- Loads data/ranking_data_upstox_30min_v3_3y.csv
...
- Saves final models, walk-forward metadata, and scaler to models/v2_30min_v3_3y/
"""
...
DATA_FILE = 'data/ranking_data_upstox_30min_v3_3y.csv'

print("=" * 60)
print("30-MIN V2 3-YEAR STANDALONE MODEL TRAINING PIPELINE")
print("Walk-Forward Validation with 3-Way Splits & Early Stopping")
print("=" * 60)

if not os.path.exists(DATA_FILE):
    print(f"[FATAL] Data file not found: {DATA_FILE}")
    print("Please run scripts/collectors/collect_upstox_30min_v3.py first.")
    sys.exit(1)
...
for i in range(min_train_months, len(unique_months) - val_test_horizon, 4):
...
for cfg in folds_config:
    ...
    print(f"    Long Edge: {metrics['long_edge']*100:+.4f}% | Short Edge: {metrics['short_edge']*100:+.4f}%")
...
print(f"  Average Spearman Rho (Long)  : {avg_long_rho:.4f}")
print(f"  Average Spearman Rho (Short) : {avg_short_rho:.4f}")
print(f"  Average Long Edge over Market: {avg_long_edge*100:+.4f}% per bar")
print(f"  Average Short Edge over Mkt  : {avg_short_edge*100:+.4f}% per bar")
...
metadata = {
    'description': 'V2 30-minute XGBoost Ranking Model',
    'type': 'ranking',
    'features': feature_cols,
    'num_features': len(feature_cols),
    'data_source': 'upstox_30min_v3_3y',
    'data_file': DATA_FILE,
    'total_rows': int(df.shape[0]),
    'walk_forward_summary': {
        'avg_long_spearman': float(avg_long_rho),
        'avg_short_spearman': float(avg_short_rho),
        'avg_long_edge': float(avg_long_edge),
        'avg_short_edge': float(avg_short_edge),
    },
...
print("\n" + "=" * 60)
print("V2 30-MIN TRAINING COMPLETE")
