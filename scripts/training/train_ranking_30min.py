"""
Train standalone 30-minute XGBoost ranking models using Walk-Forward Validation.
- Loads data/ranking_data_upstox_30min_1y.csv
- Automatically detects GPU (CUDA) availability and falls back to CPU if needed
- Performs 3-fold Walk-Forward Validation using 3-way splits (Train / Val / Test)
  to ensure honest, regime-diverse performance estimates with NO test set leakage.
...
- Saves final models, walk-forward metadata, and scaler to models/v1_30min/
"""
...
DATA_FILE = 'data/ranking_data_upstox_30min_1y.csv'

print("=" * 60)
print("30-MIN STANDALONE MODEL TRAINING PIPELINE")
print("Walk-Forward Validation with 3-Way Splits & Early Stopping")
print("=" * 60)

if not os.path.exists(DATA_FILE):
    print(f"[FATAL] Data file not found: {DATA_FILE}")
    print("Please run scripts/prepare_ranking_data_30min.py first.")
    sys.exit(1)
...
print(f"Data spans {len(unique_months)} months: {unique_months}")

# ========================================
# FEATURE SELECTION
# ========================================
exclude_cols = ['DateTime', 'DateTime_30Min', 'Query_ID', 'Ticker',
                'Open', 'High', 'Low', 'Close', 'Volume', 'Next_30Min_Return', 'YearMonth']
feature_cols = [col for col in df.columns if col not in exclude_cols]

X = df[feature_cols].values
y_returns = df['Next_30Min_Return'].values
...
def compute_spearman_rho(df_eval, score_col, invert=False):
    ...
    for qid in df_eval['Query_ID'].unique():
        ...
        if len(q_df) > 1:
            y_eval = -q_df['Next_30Min_Return'].values if invert else q_df['Next_30Min_Return'].values
...
def evaluate_ranking_performance(df_eval, long_scores, short_scores):
    ...
    for k in topk_list:
        ...
        for qid in unique_qids:
            ...
            actual_returns = q_df['Next_30Min_Return'].values
    ...
    for qid in unique_qids:
        ...
        actual = q_df['Next_30Min_Return'].values
        long_sc = q_df['long_score'].values
        short_sc = q_df['short_score'].values
...
folds_config = [
    {
        'fold': 1,
        'train_months': ['2025-05', '2025-06', '2025-07', '2025-08', '2025-09', '2025-10', '2025-11', '2025-12', '2026-01'],
        'val_months': ['2026-02'],
        'test_months': ['2026-03']
    },
    {
        'fold': 2,
        'train_months': ['2025-06', '2025-07', '2025-08', '2025-09', '2025-10', '2025-11', '2025-12', '2026-01', '2026-02'],
        'val_months': ['2026-03'],
        'test_months': ['2026-04']
    },
    {
        'fold': 3,
        'train_months': ['2025-07', '2025-08', '2025-09', '2025-10', '2025-11', '2025-12', '2026-01', '2026-02', '2026-03'],
        'val_months': ['2026-04'],
        'test_months': ['2026-05']
    }
]

# Store results for walk-forward folds
...
for cfg in folds_config:
    ...
    bst_long = xgb.train(
        params, dtrain_long, num_boost_round=500,
    ...
    bst_short = xgb.train(
        params, dtrain_short, num_boost_round=500,
    ...
    print(f"    Long Return Edge  : {metrics['long_edge']*100:+.4f}% | Short Edge: {metrics['short_edge']*100:+.4f}% per bar")
    print(f"    Combined Edge     : {metrics['combined_edge']*100:+.4f}% per bar")
...
print(f"    Top-3 Long Selections  Avg Return: {avg_long_ret*100:+.4f}% per bar")
print(f"    Top-3 Short Selections Avg Return: {avg_short_ret*100:+.4f}% per bar (Short P&L)")
print(f"    Market (Random) Pick   Avg Return: {avg_market_ret*100:+.4f}% per bar")
print(f"    Long Edge over Market  : {avg_long_edge*100:+.4f}% per bar")
print(f"    Short Edge over Market : {avg_short_edge*100:+.4f}% per bar")
print(f"    Combined Long/Short Edge: {avg_combined_edge*100:+.4f}% per bar")
print("=" * 60)
...
# All historical data up to April 2026 is Train
# May 2026 is Validation (early stopping to prevent overfitting to the latest month)
prod_train_months = ['2025-05', '2025-06', '2025-07', '2025-08', '2025-09', '2025-10', '2025-11', '2025-12', '2026-01', '2026-02', '2026-03', '2026-04']
prod_val_months = ['2026-05']
...
bst_prod_long = xgb.train(
    params, dtrain_prod_long, num_boost_round=500,
    evals=[(dtrain_prod_long, 'train'), (dval_prod_long, 'val')],
    early_stopping_rounds=50,
    verbose_eval=50
)
bst_prod_long.save_model(LONG_MODEL_PATH)
print(f"    Saved Production Long Model: {LONG_MODEL_PATH} (best iteration: {bst_prod_long.best_iteration})")
...
bst_prod_short = xgb.train(
    params, dtrain_prod_short, num_boost_round=500,
    evals=[(dtrain_prod_short, 'train'), (dval_prod_short, 'val')],
    early_stopping_rounds=50,
    verbose_eval=50
)
bst_prod_short.save_model(SHORT_MODEL_PATH)
print(f"    Saved Production Short Model: {SHORT_MODEL_PATH} (best iteration: {bst_prod_short.best_iteration})")
...
# Compute feature importance for production model
...
print("\n" + "=" * 60)
print("TOP 10 FEATURE IMPORTANCE (Prod Short Model)")
print("=" * 60)
for feat, score in list(metadata['top_features_short'].items())[:10]:
    print(f"  {feat:<30} {score:>10.2f}")

print("\n" + "=" * 60)
print("30-MIN STANDALONE WALK-FORWARD TRAINING COMPLETE")
