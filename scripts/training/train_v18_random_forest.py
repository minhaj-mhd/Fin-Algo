"""
v18 - Random Forest Classifier (Directional).

This model discards the 20bps relative target and instead 
frames the problem as an absolute binary direction classification:
Target: Will the stock move > 0 bps in the next hour? (1 or 0)

Objective: binary:logistic
Evaluation Metric: auc
Trading Threshold: > 52% probability

Usage:
    python scripts/training/train_v18_random_forest.py

Outputs:
    models/v18_random_forest_1h/
"""
...
MODEL_DIR   = 'models/v18_random_forest_1h'
COST        = 0.0010
...
# ── data ───────────────────────────────────────────────────────────────────────
print("=" * 64)
print(f"v18 RANDOM FOREST CLASSIFIER (>{int(BREAKOUT_TH*10000)} bps target)")
print("=" * 64)
...
def get_params(pos_weight: float) -> dict:
    return {
        ...
        'booster':           'gbtree',
        'num_parallel_tree': 100,        # 100 bagged trees
        'eta':               1.0,        # learning rate doesn't matter for RF but conventionally 1.0
        'max_depth':         10,         # RF needs deeper trees than GBM
        'subsample':         0.8,        # row bagging
        'colsample_bynode':  0.8,        # feature bagging
        'alpha':             1.0,
        'lambda':            2.0,
        'min_child_weight':  10,
        'random_state':      42,
...
for cfg in folds:
    ...
    bl = xgb.train(get_params(pos_weight_long), dtr_l, num_boost_round=1, verbose_eval=False)
                   
    # Train Shorts
    dtr_s = xgb.DMatrix(Xtr, label=y_short_binary[trm])
    bs = xgb.train(get_params(pos_weight_short), dtr_s, num_boost_round=1, verbose_eval=False)
...
prod_long = xgb.train(get_params(pos_weight_long), dtr_p_l, num_boost_round=1, verbose_eval=False)
prod_long.save_model(LONG_MODEL)

dtr_p_s = xgb.DMatrix(X[ptr], label=y_short_binary[ptr])
prod_short = xgb.train(get_params(pos_weight_short), dtr_p_s, num_boost_round=1, verbose_eval=False)
...
metadata = {
    'description':  'v18 - Random Forest Classifier (Directional, >0bps target)',
