"""
Build the Daily Macro Gatekeeper V2 dataset (10-year lookback).
Combines:
...
- 3-bar forward close-to-close returns label.
...
OUTPUT_CSV = "data/ranking_data_daily_macro_v2.csv"
MIN_BARS = 100
...
def main():
    print("=" * 70)
    print("DAILY MACRO DATASET BUILDER (V2)")
    print("=" * 70)
    ...
    # Make sure we have Nifty 50 and Nifty 500
    ...
    for ticker in tqdm(TICKERS, desc="Single Ticker Features"):
        ...
        # 3-bar forward close-to-close return label
        # Close(T+2)/Close(T-1) - 1. In a sorted daily series, this is shift(-3) / Close - 1
        df_feat['Label_3D'] = df_feat['Close'].shift(-3) / df_feat['Close'] - 1.0
        
        # Keep only basic details + lean features + Label
        df_feat['Ticker'] = ticker
        df_feat['DateTime'] = df_feat.index
        
        keep_cols = ['DateTime', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Volume', 'Label_3D'] + LEAN_FEATURES
    ...
    print("Merging Nifty indices & VIX...")
    ...
    print("Merging sector index returns...")
    ...
    # We join sector returns by matching DateTime and Sector_Index
    for idx_name, df_ret in sector_index_returns.items():
        # Match only rows where Sector_Index == idx_name
        df_master = pd.merge(
            df_master,
        ...
        # Assign to Sector_Return columns if Sector_Index matches
    ...
    print("Merging global macro features...")
    # Perform a sorted left join per date and forward fill
    ...
    df_master = df_master.dropna(subset=['Label_3D'])
    ...
    # Identify features for Z-scoring
    exclude_cols = {
        'DateTime', 'Query_ID', 'Ticker', 'Label_3D', 'Sector',
    ...
    # Daily cross-sectional Z-scoring
    ...
    # All indices / global macro files are read. We check if there is lookahead.
    # A row on calendar date DateTime = T-1 (e.g. 2026-06-09) must only contain data available before 09:00 IST on day T (2026-06-10).
    # Since we strictly join daily bars of date <= T-1, lookahead is mathematically impossible.
    # Assert that no feature column in row T-1 correlates 1.0 with the forward label
    corr_to_label = df_master[feature_cols].corrwith(df_master['Label_3D']).abs()
