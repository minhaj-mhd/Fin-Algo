...
# ── setup ──────────────────────────────────────────────────────────────────────
print("=" * 64)
print(f"HYBRID V10 (Ranking) + V16 (Binary Breakout) EVALUATION")
print("=" * 64)
...
def get_v16_params(pos_weight: float) -> dict:
    return {
...
for cfg in folds:
    ...
    # --- Train V16 ---
    print("  Training V16 Classifiers...")
    dtr_l_v16 = xgb.DMatrix(Xtr, label=y_long_binary[trm])
    dva_l_v16 = xgb.DMatrix(Xva, label=y_long_binary[vam])
    bl_v16 = xgb.train(get_v16_params(pos_weight_long), dtr_l_v16, num_boost_round=500,
                       evals=[(dva_l_v16, 'val')], early_stopping_rounds=50, verbose_eval=False)

    dtr_s_v16 = xgb.DMatrix(Xtr, label=y_short_binary[trm])
    dva_s_v16 = xgb.DMatrix(Xva, label=y_short_binary[vam])
    bs_v16 = xgb.train(get_v16_params(pos_weight_short), dtr_s_v16, num_boost_round=500,
                       evals=[(dva_s_v16, 'val')], early_stopping_rounds=50, verbose_eval=False)
    ...
    dte = xgb.DMatrix(Xte)
    pred_l_v16 = bl_v16.predict(dte)
    pred_s_v16 = bs_v16.predict(dte)
    ...
    metrics = {
        ...
        'B_L':  get_metrics(logicB_l_rets),  'B_S':  get_metrics(logicB_s_rets)
            res['B']['s_ret'].append(-actual[best_pass_s_idx]); res['B']['s_n'] += 1

    # Helper to calculate stats
    def calc_stats(ret_array):
        if len(ret_array) == 0:
            return 0.0, 0.0, 0.0, 0.0
        r = np.array(ret_array)
        raw_ret = float(np.mean(r))
        net_ret = raw_ret - COST
        raw_win = float(np.mean(r > 0))
        net_hit = float(np.mean(r > COST))
        return raw_ret, net_ret, raw_win, net_hit

    for name, data in res.items():
        if data['l_n'] > 0:
            raw, net, raw_w, net_h = calc_stats(data['l_ret'])
            print(f"      Longs : {data['l_n']:>4} trades | raw {raw*10000:>+5.1f}bps | net {net*10000:>+5.1f}bps | raw win {raw_w:.1%} | net hit {net_h:.1%}")
            agg_results[name]['L_ret'].extend(data['l_ret'])
        else:
            print(f"      Longs :    0 trades | raw  +0.0bps | net  +0.0bps | raw win 0.0% | net hit 0.0%")
            
        if data['s_n'] > 0:
            raw, net, raw_w, net_h = calc_stats(data['s_ret'])
            print(f"      Shorts: {data['s_n']:>4} trades | raw {raw*10000:>+5.1f}bps | net {net*10000:>+5.1f}bps | raw win {raw_w:.1%} | net hit {net_h:.1%}")
            agg_results[name]['S_ret'].extend(data['s_ret'])
        else:
            print(f"      Shorts:    0 trades | raw  +0.0bps | net  +0.0bps | raw win 0.0% | net hit 0.0%")
...
for name in ['A1', 'A3', 'B']:
    for side in ['L', 'S']:
        rets = np.array(agg_results[name][f'{side}_ret'])
        key = f"{name}_{side}"
        if len(rets) == 0:
...
out = {'cost_bps': int(COST*10000), 'breakout_th_bps': int(BREAKOUT_TH*10000),
       'prob_threshold': TRADE_PROB, 'summary': summary,
       'folds': [{'fold': i+1, **{k: list(v) for k, v in m.items()}} for i, m in enumerate(results)]}
with open('data/hybrid_v10_v16_results.json', 'w') as f:
    json.dump(out, f, indent=2)
print("\nResults saved -> data/hybrid_v10_v16_results.json")
print("=" * 64)
