"""
v15 - LambdaMART with MAP@5 Early Stopping.

This model keeps the rank:ndcg objective and quantile-based graded relevance 
of v13, but changes the early stopping evaluation metric from 'ndcg@5' to 'map@5' 
(Mean Average Precision at 5). 

Usage:
    python scripts/training/train_v15_lambdamart_map5.py

Outputs:
    models/v15_lambdamart_map5_1h/
"""
...
MODEL_DIR = 'models/v15_lambdamart_map5_1h'
COST      = 0.0010
TOP_K     = 5
...
# ── data ───────────────────────────────────────────────────────────────────────
print("=" * 64)
print("v15 LAMBDAMART - MAP@5 EARLY STOPPING")
print("=" * 64)
...
params = {
    'objective':         'rank:ndcg',
    'eval_metric':       'map@5',     # changed from ndcg@5
    'ndcg_exp_gain':     True,
    'eta':               0.03,
    'max_depth':         5,
    'subsample':         0.8,
...
for cfg in folds:
    ...
    bl = xgb.train(params, make_dmatrix(Xtr, rel_long[trm], query_ids[trm]), num_boost_round=500,
                   evals=[(make_dmatrix(Xva, rel_long[vam], query_ids[vam]), 'val')],
                   early_stopping_rounds=50, verbose_eval=False)
                   
    bs = xgb.train(params, make_dmatrix(Xtr, rel_short[trm], query_ids[trm]), num_boost_round=500,
...
# ── production models ──────────────────────────────────────────────────────────
print("\nTraining production models (Strict 80% Train, val=month before test)...")
...
metadata = {
    'description':  'v15 - LambdaMART with map@5 early stopping',
    'type':         'lambdamart_map5',
