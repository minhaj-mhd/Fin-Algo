"""
RETRAINED walk-forward for v20_rolling_1h.

Retrains the short & long rank:pairwise rankers on an EXPANDING window (faithful to
train_ranking_clean.py recipe), steps forward in 3-month OOS blocks, scores each block
with the model that was trained ONLY on prior data, and saves the concatenated
out-of-sample scored panel. Also ports the ss>0.082 floor as the training-fold 0.9992
quantile so the absolute threshold survives the change of score scale across retrains.

Output:
  scratch/wf/oos_scored.parquet   (DateTime,Ticker,Query_ID,Next_Hour_Return,ss,ls,fold,ss_thr)
  scratch/wf/fold_meta.json       (per-fold rank-IC + threshold + row counts)
"""
import os, json, time
...
PANEL = 'data/research/v21_rolling_1h_dynamic/panel.parquet'
META = 'models/research/v20_rolling_1h/metadata.json'
SS_QUANTILE = 0.9992  # what ss>0.082 corresponds to in the live model (top 0.08%)

feats = json.load(open(META))['features']
feats = feats + ['Macro_TimeOfDay', 'Macro_SP500_Ret', 'Macro_Nifty_2H']
PARAMS = {
...
# integer ranks within Query_ID (vectorised; matches rankdata 'ordinal')
...
# trade-window mask for threshold calibration (10:15-14:15)
# In v21, we REMOVE the exogenous lunch veto!
...
def group_sizes(mask):
    q = qid[mask]
    # rows already sorted by Query_ID globally; mask preserves order -> contiguous
    _, first_idx, counts = np.unique(q, return_index=True, return_counts=True)
    order = np.argsort(first_idx)
    return counts[order]

# expanding folds: first test block 2024-01, step 3 months, 3-month test, 1 val month
...
for fold, tr_m, val_m, te_m in folds:
    ...
    # training-subset mean fill (leak-free)
    ...
    # ss floor: quantile of TRAIN-window short scores
