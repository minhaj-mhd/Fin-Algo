...
def get_ticker_analysis(ticker, side_preference="NEUTRAL"):
    ...
    """
    # Support for both single key and rotated multiple keys
        ...
        try:
            ...
            try:
                ...
                if any(x in str(e_search) for x in ["429", "503", "UNAVAILABLE"]):
                    print(f"[WARN] Key {i+1} limited with Search: {e_search}. Falling back to Technical Analysis on same key...")
                    try:
                        # 2. FALLBACK WITHOUT SEARCH ON SAME KEY
                        response = client.models.generate_content(
                            model=DEFAULT_MODEL,
                        ...
                        if any(x in str(e_fallback) for x in ["429", "503", "UNAVAILABLE"]) and i < len(api_keys) - 1:
                            print(f"[WARN] Key {i+1} completely limited/unavailable. Rotating to next key...")
                            continue
                        else:
                            raise e_fallback
                else:
            ...
            if not response or not hasattr(response, 'text') or not response.text:
                ...
                return {
                    ...
                    "market_sentiment": f"Intelligence report blocked by safety filters or empty response. (Failed on API Key {i+1})",
                    "confidence": "---%",
            ...
            if "error" in data:
                return {
                    ...
                    "verdict_label": "NEUTRAL",
                    "market_sentiment": f"Gemini returned an invalid format. (Failed on API Key {i+1})",
                    "confidence": "---%",
            ...
            last_error = e
            if ("429" in str(e) or "503" in str(e) or "UNAVAILABLE" in str(e)) and i < len(api_keys) - 1:
                print(f"[WARN] Key {i+1} limited/unavailable ({str(e)}). Rotating...")
                continue
    ...
    return {
        "error": str(last_error or "All API keys exhausted"),
        "intelligence_verdict": "ERROR",
        "verdict_label": "NEUTRAL",
        "market_sentiment": f"High demand or API error. Please try again. (Exhausted {len(api_keys)} API Keys)"
    }
