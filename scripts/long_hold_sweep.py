"""
long_hold_sweep.py — Optimize holding periods and SL/TP for high average returns

Tests holding periods from 1H to 5H and End-of-Day (EOD) close,
applying a strict 0.06% transaction cost (0.06% round-trip) to see
how to maximize the average gross return per trade so it easily beats costs.
"""
...
print("=" * 80)
print("LONG-HOLD & DYNAMIC EXIT OPTIMIZATION SWEEP")
...
# Build intraday price series from 15M dataset (very precise)
price_series = {}
...
for cfg in tqdm(configs, desc="Long Hold Sweep"):
    trades = []
    ...
    for date in unique_dates:
        for T in timestamps_by_date_1h[date]:
            ...
            # Entry is at the hourly close time boundary (which corresponds to t_15m close, or the start of the next 15-min bar)
            # To get precise entry, we look up the next bar after t_15m in the 15-min price series
            for sym in top_longs:
                if sym in price_idx and t_15m in price_idx[sym]:
                    idx = price_idx[sym][t_15m]
                    series = price_series[sym]
                    entry_idx = idx + 1 # Enter at open of the next bar
                    exit_idx = min(entry_idx + cfg['hold_bars'], len(series) - 1)
                    if exit_idx > entry_idx:
                        ret = (series[exit_idx][4] / series[entry_idx][1]) - 1 # exit close vs entry open
            ...
            for sym in top_shorts:
                if sym in price_idx and t_15m in price_idx[sym]:
                    ...
                    exit_idx = min(entry_idx + cfg['hold_bars'], len(series) - 1)
                    if exit_idx > entry_idx:
                        ret = 1 - (series[exit_idx][4] / series[entry_idx][1])
                        net = ret - TRANSACTION_COST_PCT / 100
...
df_r.to_csv("data/long_hold_sweep_results.csv", index=False)

print("\n" + "=" * 120)
print(f"LONG-HOLD SWEEP RESULTS (sorted by Total Net Return, 0.06% strict transaction cost)")
print("=" * 120)
df_r_sorted = df_r.sort_values('total_return', ascending=False)
print(f"{'Config Name':<50} {'Sig/Day':>8} {'WR':>6} {'AvgGross':>10} {'AvgNet':>10} {'TotNetRet':>10} {'PF':>6} {'Green':>7}")
print(f"{'-'*50} {'-'*8} {'-'*6} {'-'*10} {'-'*10} {'-'*10} {'-'*6} {'-'*7}")
...
print(f"\n[DONE] Long-hold sweep results saved to data/long_hold_sweep_results.csv")
