"""
Fetch NATIVE 30-minute candles directly from the Upstox API and build a clean
30-minute ranking dataset. 

Why: removes all resampling ambiguity. Live inference can fetch the IDENTICAL bars.
Construction:
  * native 30m via HistoryApi.get_historical_candle_data1(ik,'30minute',to,from)
  * IST wall-clock (tz-naive), drop the 15:15 partial stub -> 12 full bars 09:15..14:45
  * session-masked Next_30Min_Return (NaN at each day's last bar -> no overnight leak)
  * same feature pipeline as production: compute_features(legacy=False) + market context + z-score

Output: data/ranking_data_upstox_30min_v3_3y.csv  (non-destructive new file)
Cache : data/raw_upstox_cache_30min_v3/{TICKER}.csv  (native 30m OHLCV per ticker)
"""
...
OUTPUT_CSV    = 'data/ranking_data_upstox_30min_v3_3y.csv'
RAW_CACHE_DIR = 'data/raw_upstox_cache_30min_v3'
START_DATE    = date(2023, 1, 1)
CHUNK_DAYS    = 90
RATE_PAUSE    = 0.25
MIN_BARS      = 100
VALID_TODS    = {'09:15', '09:45', '10:15', '10:45', '11:15', '11:45', 
                 '12:15', '12:45', '13:15', '13:45', '14:15', '14:45'}  # drop 15:15 stub
os.makedirs(RAW_CACHE_DIR, exist_ok=True)

broker = UpstoxSandboxBroker()
hist_api = upstox_client.HistoryApi(broker.data_api_client)
...
def fetch_native_30m(ticker):
    ...
    for (frm, to) in CHUNKS:
        for attempt in range(2):
            try:
                resp = hist_api.get_historical_candle_data1(ik, '30minute', to, frm, '2.0')
...
def build_ticker(ticker, raw):
    ...
    feat['Next_30Min_Return'] = session_masked_fwd(feat['Close'])
...
def build_ranking(df_all):
    ...
    df_all = df_all.dropna(subset=['Next_30Min_Return']).sort_values('DateTime')
    ...
    exclude = {'DateTime', 'Query_ID', 'Ticker', 'Next_30Min_Return', 'Open', 'High', 'Low', 'Close', 'Volume',
...
print("\nPhase 1: fetch native 30m per ticker...")
frames, ok, skip = [], 0, 0
for ticker in tqdm(TICKERS, desc='Tickers'):
    try:
        raw = fetch_native_30m(ticker)
