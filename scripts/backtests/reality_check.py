...
def main():
        ...
        for bar_idx, T in enumerate(unique_15m_times):
            ...
            if strategy_id == 6:
                ...
                for t in active_trades:
                    ...
                    # Determine strategy thresholds for 5-minute Intrabar check
                    trailing_stop = None
                    sl_pct = 0.010  # Default 1% stop-loss for all strategies
                    tp_pct = None
                    
                    if strategy_id in [7, 8]: trailing_stop = 0.004
                    elif strategy_id == 10: trailing_stop = 0.005
                    elif strategy_id in [13, 24, 25]: trailing_stop = 0.003
                    elif strategy_id == 22: trailing_stop = 0.006
                    
                    if strategy_id == 2: sl_pct = 0.006
                    elif strategy_id in [11, 12]: sl_pct = 0.006; tp_pct = 0.012
                    elif strategy_id == 18: sl_pct = 0.008; tp_pct = 0.015
                    elif strategy_id == 19: sl_pct = 0.015; tp_pct = None  # Widen S19 stop to 1.5%
                    elif strategy_id == 7 and t.get('regime') == 'HIGH_VOL': tp_pct = 0.006
                    elif strategy_id in [8, 10]: tp_pct = 0.010
                    ...
                    for t5 in sub_5m:
                        ...
                        if t['side'] == 'LONG':
                            ...
                            if sl_pct and l_pnl <= -sl_pct:
                                ...
                                break
                            if tp_pct and h_pnl >= tp_pct:
                                exit_reason = 'TAKE_PROFIT'
                                exit_price = t['entry_price'] * (1.0 + tp_pct)
                                break
                            if trailing_stop and t['peak_pnl'] - l_pnl >= trailing_stop:
                                exit_reason = 'TRAILING_STOP'
                                exit_price = t['entry_price'] * (1.0 + t['peak_pnl'] - trailing_stop)
                                break
                        else:
                            # Short side
                            ...
                            if sl_pct and high_price_pnl <= -sl_pct:
                                ...
                                exit_price = t['entry_price'] * (1.0 + sl_pct)
                                break
                            if tp_pct and low_price_pnl >= tp_pct:
                                exit_reason = 'TAKE_PROFIT'
                                exit_price = t['entry_price'] * (1.0 - tp_pct)
                                break
                            if trailing_stop and t['peak_pnl'] - high_price_pnl >= trailing_stop:
                                exit_reason = 'TRAILING_STOP'
                                exit_price = t['entry_price'] * (1.0 - (t['peak_pnl'] - trailing_stop))
                                break
                    ...
                    if not exit_reason:
                        ...
                        # Standard 15m logic (for time, conv flips, etc.)
                        if strategy_id == 1 and t['bars_held'] >= 1: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 2 and t['bars_held'] >= 4: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 3:
                            if t['bars_held'] >= 2: exit_reason = 'TIME_EXPIRY'
                            elif (bar['long_conv'] < 0 if t['side'] == 'LONG' else bar['short_conv'] < 0): exit_reason = 'CONV_FLIP'
                        elif strategy_id == 4:
                            if (bar['long_conv'] < bar['long_conv_lag1'] if t['side'] == 'LONG' else bar['short_conv'] < bar['short_conv_lag1']): exit_reason = 'CONV_REVERSAL'
                            elif t['bars_held'] >= 4: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 5 and t['bars_held'] >= 1: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 7:
                            if t.get('regime') == 'LOW_VOL' and t['bars_held'] >= 1: exit_reason = 'TIME_EXPIRY'
                            elif t.get('regime') == 'HIGH_VOL' and t['bars_held'] >= 4: exit_reason = 'TIME_EXPIRY'
                            elif t.get('regime') == 'NORMAL' and t['bars_held'] >= 2: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 8 and t['bars_held'] >= 8: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 9:
                            if (bar['spread_zscore'] < 1.0 if t['side'] == 'LONG' else bar['spread_zscore'] > -1.0): exit_reason = 'ZSCORE_EXIT'
                            elif t['bars_held'] >= 6: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 10:
                            if (bar['long_conv'] < 0 if t['side'] == 'LONG' else bar['short_conv'] < 0): exit_reason = 'CONV_FLIP'
                            elif t['bars_held'] >= 4: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id in [11, 12, 14, 16, 17, 19, 21] and t['bars_held'] >= 4: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id in [13, 18] and t['bars_held'] >= 8: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 15 and t['bars_held'] >= 2: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 23 and t['bars_held'] >= 4: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id == 20:
                            if (bar['spread_zscore'] > -1.0 if t['side'] == 'LONG' else bar['spread_zscore'] < 1.0): exit_reason = 'ZSCORE_EXIT'
                            elif t['bars_held'] >= 6: exit_reason = 'TIME_EXPIRY'
                        elif strategy_id in [24, 25] and t['bars_held'] >= 12: exit_reason = 'TIME_EXPIRY'
            ...
            if strategy_id == 1:
                if 10 * 60 <= t_min <= 14 * 60 + 59:
                    if daily_trade_count[date_str] < limit:
                        ...
                        # Place trades
                        if long_cand and daily_trade_count[date_str] < limit:
                        ...
                        if short_cand and daily_trade_count[date_str] < limit:
                            active_trades.append({
                        ...
                        if x['short_rank'] <= 5:
                            ...
                            if h1_pred and h1_pred['long_rank'] <= 0.30 * h1_pred['count']:
                                if x['short_conv'] > x['p70_short']:
                                    if ticker not in active_tickers and daily_trade_count[date_str] < limit:
                                        active_trades.append({
                                            'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
                                            'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0
                                        })
            ...
            # =================================================================
                ...
                if 13 * 60 + 45 <= t_min <= 15 * 60 + 0:
                    if daily_trade_count[date_str] < limit:
                        # Logic simplified for experiment: removed daily veto
                        for x in top_15m_longs:
            ...
            # =================================================================
                ...
                if time_str in ["10:30", "11:30", "12:30", "13:30", "14:30"]:
                    if len(active_pairs) < 2 and daily_trade_count[date_str] < limit:
                        blend_long_scores = {}
                        blend_short_scores = {}
                        for x in active_15m:
                            ticker = x['ticker']
            ...
            # =================================================================
                ...
                if curr_vol < dict_p30_vol.get(T, 0.0):
                    regime = 'LOW_VOL'
                    limit_v = 10
                elif curr_vol > dict_p70_vol.get(T, 0.0):
                    regime = 'HIGH_VOL'
                    limit_v = 4
                else:
                    regime = 'NORMAL'
                ...
                if daily_trade_count[date_str] < limit_v:
                    if regime == 'LOW_VOL':
                        for x in top_15m_longs:
                            if x['long_rank'] <= 3 and x['ibs'] < 0.45:
                                ...
                                if ticker not in active_tickers:
                                    ...
                                    active_tickers.add(ticker)
                                    daily_trade_count[date_str] += 1
                                    if daily_trade_count[date_str] >= limit_v: break
                                    
                        for x in top_15m_shorts:
                            if x['short_rank'] <= 3 and x['ibs'] > 0.55 and daily_trade_count[date_str] < limit_v:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
                        ...
                        for x in top_15m_shorts:
                            if x['short_rank'] == 1 and x['ibs'] > 0.70 and daily_trade_count[date_str] < limit_v:
                                ticker = x['ticker']
                                m30_pred = dict_30m[t_30m].get(ticker)
                        ...
                        for x in top_15m_shorts:
                            if x['short_rank'] <= 2 and x['ibs'] > 0.62 and daily_trade_count[date_str] < limit_v:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
                ...
                if t_min >= 10 * 60:
                    if daily_trade_count[date_str] < limit:
                        ...
                        for x in top_15m_shorts:
                            if x['short_rank'] <= 5 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                if ticker not in daily_tickers_traded[date_str]:
                                    h1_pred = dict_1h[t_1h].get(ticker)
            ...
            # =================================================================
                ...
                if 9 * 60 + 45 <= t_min <= 14 * 60 + 45:
                    if daily_trade_count[date_str] < limit:
                        ...
                        for x in top_15m_shorts:
                            if x['short_rank'] <= 3 and x['spread_zscore'] < -2.0 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
                ...
                if daily_trade_count[date_str] < limit:
                    ...
                    for x in top_15m_shorts:
                        if x['short_rank'] <= 3 and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            h1_pred = dict_1h[t_1h].get(ticker)
            ...
            # =================================================================
            # STRATEGY 11: "Trend Pullback Entry (Buy-the-Dip)"
            # =================================================================
            elif strategy_id == 11:
                if daily_trade_count[date_str] < limit:
                    for x in top_15m_longs:
                        ...
                        h1_pred = dict_1h[t_1h].get(ticker)
                        if h1_pred and h1_pred['long_rank'] <= 10:
                            if x['ibs'] < 0.20 and x['long_conv'] > x['long_conv_lag1']:
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                                        'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0
            ...
            # =================================================================
            # STRATEGY 12: "High-Velocity Breakout (Speed Scanner)"
            # =================================================================
            elif strategy_id == 12:
                if daily_trade_count[date_str] < limit:
                    # Long entries
                    for x in top_15m_longs:
                        if x['long_rank'] <= 5:
                            if x['long_conv'] - x['long_conv_lag1'] > 2 * x['atr_14_pct']:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                    ...
                    for x in top_15m_shorts:
                        if x['short_rank'] <= 5 and daily_trade_count[date_str] < limit:
                            if x['short_conv'] - x['short_conv_lag1'] > 2 * x['atr_14_pct']:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 13: "Midday Momentum Extension"
            # =================================================================
            elif strategy_id == 13:
                if 11 * 60 + 30 <= t_min <= 13 * 60 + 30:
                    if daily_trade_count[date_str] < limit:
                        # Long
                        for x in top_15m_longs:
                            if x['long_rank'] <= 5:
            ...
            # =================================================================
            # STRATEGY 14: "Volume-Weighted Conviction Leader"
            # =================================================================
            elif strategy_id == 14:
                if daily_trade_count[date_str] < limit:
                    # Long
                    for x in top_15m_longs:
                        if x['long_rank'] <= 5 and x['vol_rank_pct'] >= 0.90:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                    ...
                    # Short
                    for x in top_15m_shorts:
                        if x['short_rank'] <= 5 and x['vol_rank_pct'] >= 0.90 and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 15: "Overextended Conviction Fade"
            # =================================================================
            elif strategy_id == 15:
                if daily_trade_count[date_str] < limit:
                    # Enter SHORT (fade overextended long)
                    for x in top_15m_longs:
                        if x['persist_long_4bar_3'] and x['ibs'] > 0.85 and x['long_conv'] < x['long_conv_ma3']:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
                    ...
                    # Enter LONG (fade overextended short)
                    for x in top_15m_shorts:
                        if x['persist_short_4bar_3'] and x['ibs'] < 0.15 and x['short_conv'] < x['short_conv_ma3'] and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 16: "Opening Gap Fade"
            # =================================================================
            elif strategy_id == 16:
                if time_str == "09:15":
                    if daily_trade_count[date_str] < limit:
                        # Fade gap up (Enter SHORT)
                        for x in top_15m_longs:
                            if x['gap_pct'] > 0.01:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                        ...
                        # Fade gap down (Enter LONG)
                        for x in top_15m_shorts:
                            if x['gap_pct'] < -0.01 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 17: "Bollinger-Band Conviction Divergence"
            # =================================================================
            elif strategy_id == 17:
                if daily_trade_count[date_str] < limit:
                    # Fade upper band touch (Enter SHORT)
                    for x in top_15m_longs:
                        if x['high'] >= x['bb_upper'] and x['long_conv'] < x['long_conv_lag1']:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
                    ...
                    # Fade lower band touch (Enter LONG)
                    for x in top_15m_shorts:
                        if x['low'] <= x['bb_lower'] and x['short_conv'] < x['short_conv_lag1'] and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 18: "Volatility Expansion (ATR Breakout)"
            # =================================================================
            elif strategy_id == 18:
                curr_vol = dict_rolling_vol.get(T, 0.0)
                if curr_vol > dict_p70_vol.get(T, 0.0):
                    if daily_trade_count[date_str] < limit:
                        # Long
                        for x in top_15m_longs:
                            if x['long_rank'] <= 3 and x['atr_14_pct'] > 1.5:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                        ...
                        # Short
                        for x in top_15m_shorts:
                            if x['short_rank'] <= 3 and x['atr_14_pct'] > 1.5 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 19: "Low-Volatility Grind (IBS Reversion)"
            # =================================================================
            elif strategy_id == 19:
                curr_vol = dict_rolling_vol.get(T, 0.0)
                if curr_vol < dict_p30_vol.get(T, 0.0):
                    if daily_trade_count[date_str] < limit:
                        # Long
                        for x in top_15m_longs:
                            if x['long_rank'] <= 5 and x['ibs'] < 0.15:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                        ...
                        # Short
                        for x in top_15m_shorts:
                            if x['short_rank'] <= 5 and x['ibs'] > 0.85 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 20: "Mean-Reverting Conviction Spread"
            # =================================================================
            elif strategy_id == 20:
                curr_vol = dict_rolling_vol.get(T, 0.0)
                if curr_vol < dict_p50_vol.get(T, 0.0):
                    if daily_trade_count[date_str] < limit:
                        ...
                        # Short: Z-score > 2.5
                        for x in top_15m_shorts:
                            if x['spread_zscore'] > 2.5 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 21: "Conviction Persistence Anchor"
            # =================================================================
            elif strategy_id == 21:
                if daily_trade_count[date_str] < limit:
                    # Long
                    for x in top_15m_longs:
                        if x['persist_long_3bar_5']:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                    ...
                    # Short
                    for x in top_15m_shorts:
                        if x['persist_short_3bar_5'] and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 22: "Momentum Spike & Hold"
            # =================================================================
            elif strategy_id == 22:
                if daily_trade_count[date_str] < limit:
                    # Long
                    ...
                    # Short
                    for x in top_15m_shorts:
                        if x['short_rank'] <= 2 and x['short_conv'] >= x['short_conv_daily_max'] and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 23: "Pre-Close Power Hour Reversal"
            # =================================================================
            elif strategy_id == 23:
                if 14 * 60 <= t_min <= 15 * 60:
                    if daily_trade_count[date_str] < limit:
                        for x in active_15m:
                            ticker = x['ticker']
                            trend = x['close'] - x['day_open']
                            
                            # Bullish trend fade -> enter SHORT
                            if trend > 0.0 and x['short_rank'] <= 3:
                                if ticker not in active_tickers:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
                                        'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0
                            ...
                            # Bearish trend fade -> enter LONG
                            elif trend < 0.0 and x['long_rank'] <= 3:
                                if ticker not in active_tickers and daily_trade_count[date_str] < limit:
                                    active_trades.append({
                                        'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                                        'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0
            ...
            # =================================================================
            # STRATEGY 24: "Blended Conviction Ensemble"
            # =================================================================
            elif strategy_id == 24:
                if daily_trade_count[date_str] < limit:
                    # Long
                    for x in top_15m_longs:
                        if x['blend_long_rank'] <= 2:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                    ...
                    # Short
                    for x in top_15m_shorts:
                        if x['blend_short_rank'] <= 2 and daily_trade_count[date_str] < limit:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                active_trades.append({
                                    'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
            ...
            # =================================================================
            # STRATEGY 25: "Triple-Timeframe Momentum Grid"
            # =================================================================
            elif strategy_id == 25:
                if daily_trade_count[date_str] < limit:
                    # Long
                    for x in top_15m_longs:
                        if x['long_rank'] <= 5 and x['m30_long_rank'] <= 8 and x['h1_long_rank'] <= 10:
                    ...
                    # Short
                    for x in top_15m_shorts:
                        if x['short_rank'] <= 5 and x['m30_short_rank'] <= 8 and x['h1_short_rank'] <= 10 and daily_trade_count[date_str] < limit:
    ...
    strategies_info = [
        ...
        (11, "Trend Pullback Entry (Buy-the-Dip)"),
        (12, "High-Velocity Breakout (Speed Scanner)"),
        (13, "Midday Momentum Extension"),
        (14, "Volume-Weighted Conviction Leader"),
        (15, "Overextended Conviction Fade"),
        (16, "Opening Gap Fade"),
        (8, "Persistence Hunter"),
        (9, "Spread Reversion Scalper"),
        (10, "Conviction Flip Early Exit"),
        (11, "Aggressive Momentum Follower"),
        (12, "Deep Value Fade"),
        (13, "Multi-Timeframe Trend Catcher"),
        (14, "Volume Weighted Breakout"),
        (15, "Opening Gap Fade"),
        (16, "Post-Gap Continuation"),
        (17, "Late Day Squeeze"),
        (18, "Slow Grind Reversal"),
        (19, "Low-Vol Mean Reversion Squeeze"),
        (20, "High-Vol Breakout Fade"),
        (21, "Intraday Trend Continuation"),
        (22, "Extreme Exhaustion Fade"),
        (23, "Power Hour Reversal"),
        (24, "Blended Conviction Ensemble"),
    ...
    output_path = "data/strategy_25x_results.json"
    ...
    print("\n" + "=" * 145)
    print("BACKTEST SIMULATION SUMMARY TABLE (25 STRATEGIES)")
    print("=" * 145)
