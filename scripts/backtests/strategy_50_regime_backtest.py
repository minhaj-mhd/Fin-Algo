...
def evaluate_strategy_trades(trades, name, slots=5):
    ...
    # Capital-Adjusted Returns (Disabled division to match raw undivided returns of 10x backtest baseline)
        ...
        for bar_idx, T in enumerate(unique_15m_times):
            ...
            if strategy_id == 6:
                ...
                for t in active_trades:
                    ...
                    # Extract strategy-specific exit parameters
                    trailing_stop = None
                    tp_pct = None
                    sl_pct = 0.010  # Default 1.0% stop loss fallback
                    
                    if strategy_id in [1, 3, 4, 5, 9, 11]:
                        sl_pct = None
                    elif strategy_id == 2:
                        sl_pct = 0.006
                    elif strategy_id == 12:
                    ...
                    if not exit_reason:
                        ...
                        # Check custom strategy microstructural exits
                        if strategy_id == 3:
                            ...
                            z_pivot = (bar['spread_zscore'] <= 0 if t['side'] == 'LONG' else bar['spread_zscore'] >= 0)
                            if z_pivot:
                            ...
                            # Standard table-driven time expiry fallback
                    ...
                    if exit_reason:
                        ...
                        if exit_reason == 'STOP_LOSS':
                            ...
                            elif strategy_id in [11, 12]:
                                exit_price = t['entry_price'] * (0.994 if t['side'] == 'LONG' else 1.006)
                            elif strategy_id == 18:
                                exit_price = t['entry_price'] * (0.992 if t['side'] == 'LONG' else 1.008)
                            ...
                            if strategy_id == 7:
                                ...
                                exit_price = t['entry_price'] * (1.012 if t['side'] == 'LONG' else 0.988)
                            elif strategy_id == 18:
                                exit_price = t['entry_price'] * (1.015 if t['side'] == 'LONG' else 0.985)
                            ...
                            if strategy_id in [7, 8]:
                                ...
                                exit_price = t['entry_price'] * (1.0 + t['peak_pnl'] - 0.003) if t['side'] == 'LONG' else t['entry_price'] * (1.0 - (t['peak_pnl'] - 0.003))
                            elif strategy_id == 22:
                                exit_price = t['entry_price'] * (1.0 + t['peak_pnl'] - 0.006) if t['side'] == 'LONG' else t['entry_price'] * (1.0 - (t['peak_pnl'] - 0.006))
            ...
            if apply_gatekeeper and strategy_id != 6 and d_daily and d_daily in dict_daily:
                filtered_longs = []
                    ...
                    if daily_trade_count[date_str] < limit:
                        for x in top_15m_longs:
                            if x['gap_pct'] < -0.01 and x['h1_long_rank'] <= 3 and x['daily_long_rank'] <= 3:
                                ticker = x['ticker']
                                if ticker not in active_tickers:
                                    append_trade(active_trades, {'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'], 'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0})
                                    active_tickers.add(ticker)
                                    daily_trade_count[date_str] += 1
                                    if daily_trade_count[date_str] >= limit: break
                ...
                if daily_trade_count[date_str] < limit:
                    ...
                    for x in top_15m_shorts:
                        ...
                        if x['short_rank'] <= 10 and x['short_conv'] > x['short_conv_lag1'] and x['short_conv_lag1'] > x['short_conv_lag2'] and x['short_conv_lag2'] > 0:
                            ticker = x['ticker']
                            if ticker not in active_tickers:
                                append_trade(active_trades, {'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'], 'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0})
                                active_tickers.add(ticker)
                                daily_trade_count[date_str] += 1
