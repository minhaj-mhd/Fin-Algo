...
# ========================================
TEST_MONTH = "2026-05"
TRANSACTION_COST_PCT = 0.15  # 0.06% round-trip (including slippage)
...
def main():
                    ...
                    if daily_trade_count[date_str] < limit:
                        ...
                        for x in top_15m_longs:
                            if x['gap_pct'] > 0.01:
                                ticker = x['ticker']
                                d_pred = dict_daily[d_daily].get(ticker)
                                if d_pred and d_pred['short_rank'] <= 10:
                                    if ticker not in active_tickers:
                                        active_trades.append({
                                            'ticker': ticker, 'side': 'SHORT', 'entry_price': x['close'],
                                            'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0
                        ...
                        for x in top_15m_shorts:
                            if x['gap_pct'] < -0.01 and daily_trade_count[date_str] < limit:
                                ticker = x['ticker']
                                d_pred = dict_daily[d_daily].get(ticker)
                                if d_pred and d_pred['long_rank'] <= 10:
                                    if ticker not in active_tickers:
                                        active_trades.append({
                                            'ticker': ticker, 'side': 'LONG', 'entry_price': x['close'],
                                            'entry_time': T, 'bars_held': 0, 'peak_pnl': 0.0
    ...
    strategies_info = [
        ...
        (16, "Opening Gap Fade"),
        (17, "Bollinger-Band Conviction Divergence"),
        (18, "Volatility Expansion (ATR Breakout)"),
        (19, "Low-Volatility Grind (IBS Reversion)"),
        (20, "Mean-Reverting Conviction Spread"),
        (21, "Conviction Persistence Anchor"),
        (22, "Momentum Spike & Hold"),
        (23, "Pre-Close Power Hour Reversal"),
