...
GATE_PCT     = 0.10
# Daily macro gate (live gatekeeper): a LONG candidate must be in long_eligible, a SHORT in
# short_eligible. WARNING: data/daily_gatekeepers.json is a SINGLE snapshot; the live gate is
# recomputed every day, so applying one snapshot across the whole OOS window is LOOKAHEAD.
# Faithful use needs per-date gatekeeper history. Set None to disable.
...
# ── BANNER ─────────────────────────────────────────────────────────────────────
print("=" * 70)
print("  v20 1H + v3 15min  |  TRUE OOS Backtest  |  Jun 5-18, 2026")
...
# ── DAILY MACRO GATE (eligibility shortlist) ────────────────────────────────────
long_elig = short_elig = None
if DAILY_GATE_FILE:
    with open(DAILY_GATE_FILE) as f: _gk = json.load(f)
    long_elig  = set(_gk.get("long_eligible", []))
    short_elig = set(_gk.get("short_eligible", []))
    print(f"     daily gate   : {len(long_elig)} long / {len(short_elig)} short eligible "
          f"(snapshot {_gk.get('timestamp','?')})")
    print( "     [WARN] single-snapshot gate applied to the WHOLE window = LOOKAHEAD; "
           "directional what-if only, not a valid OOS result.")
...
for i, ts in enumerate(all_ts):
    ...
    for side in ("LONG", "SHORT"):
        ...
        # Daily macro gate: keep only tickers eligible for this side (see DAILY_GATE_FILE note)
        elig = long_elig if side == "LONG" else short_elig
        if elig is not None:
            cands = cands[cands["Ticker"].map(lambda t: t.replace(".NS", "") in elig)]
