...
**Author context:** Written after the v8–v19 demotion arc. See [[02 — Models/_Shared/Model Performance & Statistics]].
...
The 48-feature normalization strategy in [[02 — Models/_Shared/Feature Engineering & Normalization]] was *designed* for cross-sectional comparability (percentage distances, bounded oscillators, OBV rate-of-change) and is reused as-is, with two changes:
...
All features computed on **completed bars only** (the IBS same-bar leakage lesson — see [[06 — Logs/Daily Logs/2026-06-08|2026-06-08 diagnosis]]).
...
Under the 3-year mandate ([[02 — Models/_Shared/Training Data & Regime Requirements]]), ~750 trading days:
...
3. **Cost model**: 6 bps per side, applied per side, with the mandatory `median(net − gross) == −cost` per-side audit ([[06 — Logs/Daily Logs/2026-06-09|TBM cost-sign lesson]]). Report RAW and NET win-rates separately for long and short.
...
5. **Final verdict only via the Validation Gauntlet** (per [[01 — Architecture/Validation Gauntlet/Validation Gauntlet Architecture]]). Note: the Gauntlet's XGBoost-centric harness will need a `predict`-adapter for a PyTorch artifact — scope this before Stage 2. Any metric quoted without a Gauntlet `run_id` stays ⚠️ UNVERIFIED.
...
- **What this does not fix**: the 6 bps cost floor and the near-efficiency of bar data. The larger expected payoff remains richer inputs — order-book imbalance, aggressor flow, futures basis — per the Market Psychology roadmap item in [[06 — Logs/Active Board]].
...
## Backlinks
- [[02 — Models/_Shared/Model Performance & Statistics]] — incumbent baselines and raw-signal audit this proposal must beat.
- [[02 — Models/_Shared/Feature Engineering & Normalization]] — reused feature pipeline.
- [[02 — Models/_Shared/Training Data & Regime Requirements]] — 3-year mandate / Row Count Fallacy governing the size budget.
- [[01 — Architecture/Validation Gauntlet/Validation Gauntlet Architecture]] — sole verdict authority.
