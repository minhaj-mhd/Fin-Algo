...
> **Parent spec**: [[01. Core Architecture/Validation Gauntlet Architecture]] — read it FIRST; all conventions there still bind.
...
1. **Universe-baseline z-test**: baseline per side per period = raw WR of **all** OOS candidate rows in that period (long: `(y > 0).mean()` over recent OOS rows; short: `(−y > 0).mean()`). Then
   ...
   This matches the methodology of the trusted Raw-Signal-Only Audit (see [[02. Model Suite/Model Performance & Statistics]]). Compute baseline inside `cli.py` from `harness_res` and pass it through; `evaluate_verdict` takes `baseline_wr` as an argument — no hardcoded 0.5 anywhere.
...
2. **Expected-vs-actual check** (do not skip): v8 long → expected DEAD; v8/v10 short → expected FILTER_GRADE; any TRIGGER_GRADE anywhere is a red flag demanding manual review before celebrating. Compare every verdict against [[02. Model Suite/Model Performance & Statistics]] expectations; discrepancies are findings to report, not noise.
...
## 🔗 Backlinks
- Parent spec: [[01. Core Architecture/Validation Gauntlet Architecture]]
- Audit findings: [[06. Context & Logs/Conversations/Conv-2026-06-10-Signal-Layer-Rectification-Plan]]
- Verdict expectations source: [[02. Model Suite/Model Performance & Statistics]]
