"""
Prepare ranking data for the 15-minute standalone XGBoost model.
- Loads cached 15-minute candles from data/raw_upstox_cache_15min/
- Computes standard + 15-min specific features using compute_features_15min()
- Creates Next_15Min_Return labels and Query_IDs (grouped by 15-minute floor)
...
- Saves to data/ranking_data_upstox_15min_1y.csv
"""
...
from scripts.feature_utils import compute_features_15min

# ========================================
# CONFIG
# ========================================
INPUT_CACHE_DIR = "data/raw_upstox_cache_15min"
OUTPUT_CSV = "data/ranking_data_upstox_15min_1y.csv"

print("=" * 60)
print("15-MIN DATA PIPELINE — Feature Engineering & Preparation")
print("=" * 60)
...
for file_path in tqdm(cache_files, desc="Processing Tickers"):
    ...
    try:
        ...
        # Compute 15-minute features
        df = compute_features_15min(df, legacy=False)
        
        # Target: return of the next 15-min bar
        df['Next_15Min_Return'] = df['Close'].shift(-1) / df['Close'] - 1
...
# Create query_id: group by 15-minute floor
...
returns = df_all['Next_15Min_Return']
print(f"  Label (Next_15Min_Return) stats:")
