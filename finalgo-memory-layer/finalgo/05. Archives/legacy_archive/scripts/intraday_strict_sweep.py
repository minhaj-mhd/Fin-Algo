"""
intraday_strict_sweep.py — Optimize blended ensembles under strict same-day intraday force-close

Signals are generated on hourly boundaries (10:30, 11:30, 12:30, 13:30, 14:30).
All trades are strictly closed at either:
  1) The target hold time (e.g. 1H, 2H, 3H, 4H)
  2) Or the 15:15 IST bar of the SAME day (EOD force-close).
NO overnight positions are carried.
Strict 0.06% round-trip transaction costs are applied.
"""
...
print("=" * 80)
print("STRICT INTRADAY FORCE-CLOSE OPTIMIZATION SWEEP")
print(f"Applying strict {TRANSACTION_COST_PCT}% transaction costs & 15:15 EOD force-close")
print("=" * 80)
...
# Test parameters
holding_options = [4, 8, 12, 16, 24] # 1H, 2H, 3H, 4H, 6H hold max (force-closed EOD)
topk_options = [3, 5, 10]
model_configs = [
    ('1H_Only', 1.0, 0.0, 0.0),
...
for cfg in tqdm(configs, desc="Intraday Strict Sweep"):
    trades = []
        ...
        for T in timestamps_by_date_1h[date]:
            ...
            for sym in top_longs:
                if sym in price_idx and t_15m in price_idx[sym]:
                    ...
                    # Find exit_idx: either entry_idx + hold_bars OR the 15:15:00 bar of the same day
                    exit_idx = entry_idx
                    for offset in range(1, cfg['hold_bars'] + 1):
                        curr_idx = entry_idx + offset
                        if curr_idx >= len(series):
                            break
                        exit_idx = curr_idx
                        # Force-close on 15:15 bar
                        if series[curr_idx][0].endswith("15:15:00+05:30"):
                            break
                            
                    if exit_idx > entry_idx:
                        ret = (series[exit_idx][1] / series[entry_idx][1]) - 1
...
df_r.to_csv("data/intraday_strict_sweep_results.csv", index=False)

print("\n" + "=" * 120)
print(f"STRICT INTRADAY SWEEP RESULTS (sorted by Total Net Return, EOD close @ 15:15, 0.06% costs)")
print("=" * 120)
...
print(f"\n[DONE] Strict intraday sweep results saved to data/intraday_strict_sweep_results.csv")
